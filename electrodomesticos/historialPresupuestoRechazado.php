...
<body>
    <h1 class="titulo">Historial de Presupuestos Rechazados</h1>
    <hr>

    <!-- Presupuestos rechazados -->
    <div id="presuRechazado" class="grid-container" >
        <div class="div-con-botones" style="grid-column: 1 / -1; text-align:center;">
            ...
            <button class='btn' id="btn-filtro" type="button" onclick="mostrarFiltros('filtrosPresu')">
                Filtros
        ...
        foreach ($reparaciones as $rep):
            if ($rep->getEstadoPresu() == 'Presupuesto Rechazado'): 
                $contador_reparaciones++;
                $fecha_ing_electro= date("d/m/Y", strtotime($rep->getFechaIngElectro()));
                $fecha_env_presu = date("d/m/Y", strtotime($rep->getFechaEnvioPresup()));
                $fecha_rechaza = date("d/m/Y", strtotime($rep->getFechaConfirmReparacion()));
        ...
        ?>
            <div class="card" onclick="expandirDetalle(<?= $rep->getIdReparacion() ?>)">
                ...
                <div class="card-header">Reparación Rechazada #<?= $rep->getIdReparacion() ?></div>
                <ul class="list-group">
                    <li class="list-group-item"><strong>Fecha Ingreso de Electro:</strong> <?= $fecha_ing_electro ?></li>
                    <li class="list-group-item"><strong>Fecha Envío Presupuesto:</strong> <?= $fecha_env_presu ?></li>
                    <li class="list-group-item"><strong>Fecha que Rechazó el Presupuesto:</strong> <?= $fecha_rechaza ?></li>
                    ...
                    <li class="list-group-item"><strong>Arancel Inicio Abonado:</strong> $<?= $monto_fijo ?> <strong> Medio de Pago:</strong><?= ucwords($rep->getMedioPagoIni())?></li>
        ...
        <?php if ($contador_reparaciones == 0): ?>
            <div style="grid-column: 1 / -1; text-align: center;">
                <h3>No hay Historial de Presupuestos Rechazados</h3>
            </div>
        <?php endif; ?>
    </div>
    ...
    <?php
     if($busquedaRealizada){
            echo "<br><button class='btn' style='display:block; margin-left: auto; margin-right:auto; margin-bottom: 15px;'><a style='text-decoration:none; color:white;' href='historialPresupuestoRechazado.php'>Volver</a></button>";
        }
    ?>
                    ...
                    contenido.innerHTML = `
                        ...
                        ?>
                            ...
                            <ul class="list-group">
                                <li class="list-group-item"><strong>Fecha Envío Presupuesto:</strong> <?= $fecha_env_presu ?></li>
                                <li class="list-group-item"><strong>Fecha que Rechazó el Presupuesto:</strong> <?= $fecha_rechaza ?></li>
                                <li class="list-group-item"><strong>Detalles de la reparación ofrecida:</strong> <?= ucwords($detalleReparacion)  ?></li>
                                <li class="list-group-item"><strong>Presupuesto:</strong> $<?= $presup ?></li>
                                <li class="list-group-item"><strong>Arancel Inicio Abonado:</strong> $<?= $monto_fijo ?> <strong> Medio de Pago:</strong><?= ucwords($reparacion->getMedioPagoIni())?></li>
